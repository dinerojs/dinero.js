...
let safeAllocate = (calculator: calculator<'amount>) => {
  ...
  (dineroObject: dinero<'amount>, ratios: array<ScaledAmount.scaledAmount<'amount>>) => {
    ...
    let hasOnlyPositiveRatios =
      normalizedRatios->Array.every(ratio => greaterThanOrEqualFn(ratio.amount, zero))
    let hasOneNonZeroRatio =
      normalizedRatios->Array.some(ratio => greaterThanFn(ratio.amount, zero))
