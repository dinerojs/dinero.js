...
type greaterThanOrEqualParams<'amount> = (dinero<'amount>, dinero<'amount>)

let unsafeGreaterThanOrEqual = (calculator: calculator<'amount>) => {
  ...
  (dineroObject: dinero<'amount>, comparator: dinero<'amount>) => {
    ...
    greaterThanOrEqualFn(subjectAmount, comparatorAmount)
  }
}

let safeGreaterThanOrEqual = (calculator: calculator<'amount>) => {
