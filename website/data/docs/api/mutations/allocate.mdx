...
Monetary values have indivisible units, meaning you can't always exactly split them. A good example is invoicing: let's say you need to bill $999.99 and you want a 50% downpayment. You can't actually split such an amount in two "billable" amounts, because you can't split a penny. With `allocate`, you can split a monetary amount then distribute the remainder as evenly as possible.
...
If you need to use fractional ratios, you shouldn't use floats, but scaled ratios instead. For example, instead of passing `[50.5, 49.5]`, you should pass `[505, 495]` and a `scale` of `1` in `options`. When using scaled ratios, the function converts the returned objects to the safest scale.
...
<Parameter name="ratios" type="TAmount[]" required={true}>

The ratios to allocate the amount to.

</Parameter>

<Parameter name="options" type="AllocateOptions<TAmount>" required={false}>
...
const [d1, d2] = allocate(d, [505, 495], { scale: 1 }); // translates to ratios 50.5 and 49.5
