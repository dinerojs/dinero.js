...
The `currency` is now expressed as a currency object and no longer as a string. v2 provides ISO 4217 currency objects out of the box via the `@dinero.js/currencies-number` package.
...
| `d1.toUnit(...args)`        | [`toUnit(d1, ...args)`](/docs/api/formatting/to-unit)                |
| `d1.toRoundedUnit(...args)` | [`toUnit(d1, ...args)`](/docs/api/formatting/to-unit)                |
