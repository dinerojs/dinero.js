...
You can display Dinero objects into any format, exactly the way you want. Dinero.js lets you build a formatter that exposes a pre-formatted amount, in units and in stringified decimal format (if the currency is decimal and single-base).

```js
import { dinero, toFormat } from 'dinero.js';
import { USD, MGA } from '@dinero.js/currencies';

const d1 = dinero({ amount: 1150, currency: USD });
const d2 = dinero({ amount: 13, currency: MGA });

toFormat(d1, ({ decimal, currency }) => `${currency.code} ${decimal}`); // "USD 11.5"
