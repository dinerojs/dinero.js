...
The Dinero.js library provides the `@dinero.js/currencies-number` package so you can have access to currency objects out of the box.
...
// Node.js
const { USD } = require('@dinero.js/currencies-number');
```

If you don’t use a package manager, you can use the HTML `script` element:

```bash
<script src="https://cdn.jsdelivr.net/npm/@dinero.js/currencies-number@alpha/dist/umd/index.production.js"></script>
<script>
  const { USD } = window['@dinero.js/currencies-number'];
</script>
```
