...
Dinero objects are ideal to safely manipulate money, but at some point, you need to display them. The `toFormat` function lets you display objects the way you want. It exposes a pre-formatted amount in `units` and in stringified `decimal` format (if the currency is decimal and single-base), for convenience.

```js
import { dinero, toFormat } from 'dinero.js';
import { USD } from '@dinero.js/currencies';

const transformer = ({ decimal, currency }) => `${currency.code} ${decimal}`;

const d = dinero({ amount: 5000, currency: USD });

toFormat(d, transformer); // "USD 50.00"
```
