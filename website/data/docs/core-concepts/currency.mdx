...
Dinero.js provides ISO 4217 currency objects out of the box via the `@dinero.js/currencies-number` package.
...
import { USD, EUR } from '@dinero.js/currencies-number';
...
Dinero.js tracks those amendments and updates currencies accordingly. Such changes are, by definition, breaking. To preserve fixed versioning, the library's entry point doesn't export updated currencies. In other words, if you import currencies from the path `@dinero.js/currencies-number`, they won't change.
...
import { USD } from '@dinero.js/currencies-number/latest';
...
```js
import { USD } from '@dinero.js/currencies-number/168';
...
You can build your own currency object if it isn't available in `@dinero.js/currencies-number`.
