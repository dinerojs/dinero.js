...
While certain programming languages like Java or C# safely support arbitrary precision decimals, JavaScript only lets you natively express decimals is with using [double precision floats](https://wikipedia.org/wiki/IEEE_754). The limitation is that it uses a binary system to represent decimal numbers, so you end up with inaccurate results when trying to math.
...
```js
const purchase = {
  title: 'Eccotemp i12-NG Indoor Natural Gas Tankless Water Heater',
  price: 369.99,
};
```
...
You can also combine it with utility libraries such as Ramda if that's your jam.
