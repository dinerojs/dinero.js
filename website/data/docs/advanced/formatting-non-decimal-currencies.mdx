...
In addition to the formatted amount and the currency, the `createFormatter` function also exposes the original object. You can leverage it to build a more complex formatter.

```jsx
import { dinero, createFormatter, toSnapshot } from 'dinero.js';
import pluralize from 'pluralize';

// ...

const format = createFormatter(({ dineroObject }) => {
