...
The [Paypal](https://www.paypal.com/) payment platform provides APIs to process payments and manage orders. Unlike most platforms, it expects a money representation with an amount in major currency units. You can use [`toDecimal`](/docs/api/formatting/to-decimal) to retrieve this value.
...
function toPaypalMoney(dineroObject) {
  ...
  return {
    value: toDecimal(dineroObject, ({ value }) => value),
